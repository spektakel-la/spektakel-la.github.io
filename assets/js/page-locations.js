(function () {
  const createIconMarkup = (locationObj) => {
    ...
    if (locationObj.location_label === "i") {
      ...
      label = locationObj?.location_label
        ? locationObj.location_label
  ...
  const sanityCheckSchedule = () => {
    spektakel.constants.SCHEDULE.forEach((entry) => {
      const locationForEntry = spektakel.constants.LOCATIONS.find(
        (loc) => entry.location_id === loc.location_id
      );
      if (!locationForEntry) {
        throw new Error(
          `Schedule entry with invalid location_id: '${JSON.stringify(entry)}'`
        );
      }
      const artistForEntry = spektakel.constants.ARTISTS.find(
        (art) => entry.artist_id === art.artist_id
      );
      if (!artistForEntry) {
        throw new Error(
          `Schedule entry with invalid artist_id: '${JSON.stringify(entry)}'`
        );
      }
    });
  ...
  const createPopupMarkupForLocation = (locationObj, lang) => {
    ...
    const schedule = locationObj.nightlife
      ? spektakel.constants.NIGHTLIFE
      : spektakel.constants.SCHEDULE;
    const scheduleForLocation = schedule.filter(
     ...
     */
    const prunedScheduleForLocation = scheduleForLocation.reduce(
      (acc, entry) => {
        ...
        if (
          ...
          dateFns.differenceInHours(
            ...
            dateFns.parseISO(maybeLastEntry.time)
          ) < 1
        ) {
          return acc;
        } else {
    ...
    const scheduleForLocationWithArtist = prunedScheduleForLocation.map(
      (entry) => {
        const artist = spektakel.constants.ARTISTS.find(
          (artist) => artist.artist_id === entry.artist_id
        );
        if (artist) {
          entry.artist_name = artist?.short_name
            ? artist?.short_name
            : artist?.name;
    ...
    const tableRows = scheduleForLocationWithArtist
      .map((sched, idx) => {
        const now = new Date();
        ...
        if (
          currentDateString !== now.toLocaleDateString() && // don't show date-section for today
          currentDateString !== sectionDateString
        ) {
          maybeDateSection = `
                    <tr class="day-section">
                        <td colspan="2">
                            <strong>${dateFns.format(
                              scheduleDate,
                              "EEEE dd.MM.yyyy",
        ...
        return `
                ...
                <tr class="schedule-details${
                  artistFavorites.includes(sched.artist_id) ? " favorite" : ""
                }"
                    data-artist-id="${
                      sched.artist_id
                    }" onclick="spektakel.locations.toggleArtistDetails(this);">
                    <td>
                ...
                <tr class="artist-details">
                    <td colspan="2">
                        ${spektakel.popover.createPopoverContentHtml(
                          sched.artist_id,
                          lang
                        )}
                    </td>
  ...
  const createPopupMarkup = (locationObj, lang) => {
    const linkToProgram = locationObj.nightlife
      ? "/nightlife"
      : `/program#${locationObj.location_id}`;
    return `
  ...
  const setupLeafletMap = async (mapContainer, lang) => {
    ...
    const tilesLayer = L.tileLayer(
      `/assets/img/map/tiles/{z}/{x}/{y}.${hasWebpSupport ? "webp" : "jpg"}`,
    ...
    const geoLocation = L.control.locate({
      strings: {
        ...
        outsideMapBoundsMsg:
          "Du scheinst Dich außerhalb der verfügbaren Kartenregion zu befinden",
      },
    });
    ...
    const markers = spektakel.constants.LOCATIONS.map((location) => {
      const markerColor = location.marker_color
      ...
      marker
        .bindPopup("", {
          className: "spektakel-leaflet-popup",
          offset: [0, -38],
        })
        .on("popupopen", (event) => {
    ...
    map.fitBounds(
      L.latLngBounds(
        spektakel.constants.LOCATIONS.map((location) => location.gps)
      )
    );
  };
