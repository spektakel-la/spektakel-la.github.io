...
for (const route of ['/', '/program/', '/artists/', '/artists/alikindoi/', '/locations/', '/impressions/', '/sponsors/', '/imprint/']) {
...
test('liefert valides Festival-, Event- und Künstler-JSON-LD', async ({ page }) => {
  for (const [route, expectedType] of [['/', 'Festival'], ['/program/', 'EventSeries'], ['/artists/alikindoi/', 'Person']] as const) {
    await page.goto(route);
    ...
    if (route !== '/artists/alikindoi/') {
      expect(jsonLd.subEvent.length).toBeGreaterThan(0);
